...
export default function PlacesManagement() {
  ...
  async function handleSave() {
    ...
    const placeData = {
      ...
      ...(editingPlace ? {} : { sort_order: places.length }),
    };
 ...
 async function handleDelete(place: Place) {
  ...
  async function movePlace(place: Place, direction: "up" | "down") {
    ...
    const { error: err1 } = await supabase
      .from("places").update({ sort_order: b.sort_order }).eq("id", a.id);
    const { error: err2 } = await supabase
      .from("places").update({ sort_order: a.sort_order }).eq("id", b.id);

    if (err1 || err2) { alert("Failed to reorder"); return; }
    loadPlaces();
  }
        ...
        <div className="places-grid">
          {places.map((place) => (
            <div key={place.id} className="place-card">
              ...
              <div className="place-actions">
              <button onClick={() => movePlace(place, "up")} className="edit-btn" title="Move up">↑</button>
              <button onClick={() => movePlace(place, "down")} className="edit-btn" title="Move down">↓</button>
              <button onClick={() => openEditModal(place)} className="edit-btn" title="Edit">Edit</button>
              <button onClick={() => handleDelete(place)} className="delete-btn" title="Delete">Delete</button>
              </div>
